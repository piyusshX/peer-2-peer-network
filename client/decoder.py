...
def load_and_decode(torr_file):
    status, *rest = load(torr_file)
    if status == 0: # loaded successfully
        stem, parent, source = rest[0], rest[1], rest[2]
        status, *rest = main(source)
        if status == 0: # decoded successfully
            return rest[0] # return the decoded bytes
        else:
            message = f'failure!\n{rest[-1]}'
            return 1, message
    else:
...
if __name__=="__main__":
    ...
    status, *rest = load('c:/Projects/torrent/peer-2-peer-network/examples/1134459.torrent')
